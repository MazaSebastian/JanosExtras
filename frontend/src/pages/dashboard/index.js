  ...
  const loadHomeData = async () => {
    try {
      ...
      const completedPre = allCoordinaciones
        .filter(c => c.pre_coordinacion_completado_por_cliente && c.estado !== 'completado')
      ...
      const week = allCoordinaciones
        .filter(c => {
          ...
          } else if (c.fecha_evento instanceof Date) {
            datePart = c.fecha_evento.toISOString().split('T')[0];
          }
          if (!datePart) return false;
          const eventDate = new Date(datePart + 'T00:00:00'); // Evitar desfasajes
